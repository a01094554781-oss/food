...
st.markdown("현재 위치 주변의 음식점을 평점, 카테고리, 지도 스타일로 탐색하거나, 직접 음식점을 추가해보세요!")
...
with st.form("add_restaurant_form"):
    ...
    with col2:
        lat = st.number_input("위도", min_value=37.45, max_value=37.60, value=37.55, step=0.0001)
        lon = st.number_input("경도", min_value=126.90, max_value=127.10, value=127.00, step=0.0001)
        address = st.text_input("주소 (간단히 입력 가능)")
    submitted = st.form_submit_button("추가하기")

    if submitted:
        new_row = pd.DataFrame([{
            "이름": name, "카테고리": category_new, "위도": lat, "경도": lon,
            "평점": rating, "주소": address
        }])
