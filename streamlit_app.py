...
st.title("🍽️ 서울 음식점 혼잡도 지도")
st.caption("혼잡도, 평점, 현재 위치 기준으로 음식점을 찾아보세요!")
...
data["color"] = data["혼잡도"].map(color_map)
...
# --- 혼잡도별 요약 ---
st.markdown("### 📊 혼잡도별 음식점 분포")
st.bar_chart(data["혼잡도"].value_counts())

st.success("✅ 지도는 마우스로 확대/이동이 가능합니다!")
